...
3.  读取 0x1018:03 — 验证固件版本   (v8 ~ v9)
4.  读取 0x1018:04 — 记录序列号
5.  读取 0x6076:00 — 读取峰值扭矩   (REAL32, mNm)
6.  写入 0x6040:00 — CiA402 状态机复位 (0 → 0x80)
7.  写入 0x1016:01 — 禁用消费者心跳监控
8.  写入 0x2040:00 — 失能时短接制动
9.  写入 0x6072:00 — 最大扭矩限幅
10. [MIT 模式] 配置 0x2004 子索引 (范围 + KP/KD 限幅)
11. 配置 TPDO1 / TPDO2 映射
12. 配置 RPDO1 映射 (共享 COB-ID 0x190 + 动态偏移填充)
13. 写入 0x1016:01 — 启用消费者心跳监控 (超时 250ms)
14. 写入 0x6060:00 — 设置工作模式 (MIT=5, Velocity=3)
15. CiA402 状态机: Shutdown(6) → SwitchOn(7) → Enable(0xF)
