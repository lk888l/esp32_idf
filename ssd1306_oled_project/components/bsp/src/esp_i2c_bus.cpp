...
I2CStatus fromEspError(esp_err_t error)
{
    switch (error) {
    case ESP_OK:
        return I2CStatus::ok;
...
public:
    ...
    I2CStatus write(std::span<const std::uint8_t> data) override
    {
        ...
        return fromEspError(i2c_master_transmit(
            handle_, data.data(), data.size(), kTransactionTimeoutMs));
    }

    I2CStatus writeRead(std::span<const std::uint8_t> write_data,
...
I2CStatus EspI2CBus::init()
{
    ...
    if (config_.sda == GPIO_NUM_NC || config_.scl == GPIO_NUM_NC) {
        return I2CStatus::invalid_argument;
    }
...
I2CDeviceResult EspI2CBus::createDevice(std::uint8_t address,
                                        std::uint32_t clock_hz)
{
    if (bus_ == nullptr) {
        return {I2CStatus::invalid_state, nullptr};
    }
    if (address > 0x7F || clock_hz == 0) {
    ...
    const I2CStatus status = fromEspError(
        i2c_master_bus_add_device(bus_, &device_config, &handle));
    if (status != I2CStatus::ok) {
